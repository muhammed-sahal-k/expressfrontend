...
function Update() {
  ...
  return (
    <div className="getdata-container">
      ...
      <table className="products-table" border="1" cellPadding="10">
        <thead>
          <tr>
            <th>Image</th>
            <th>name</th>
          ...
          {data.map((item) => (
            <tr key={item._id}>
              ...
              <td>
                {editId === item._id ? (
                  <>
                    <button onClick={() => handleUpdate(item._id)} className="action-btn update-btn">
                      Update
                    </button>
                    <button onClick={() => setEditId(null)} className="action-btn cancel-btn">
                      Cancel
                    </button>
                  </>
                ) : (
